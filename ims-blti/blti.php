...
class BLTI {
    ...
    function getUserShortName() {
        ...
        $givenname = $this->info['lis_person_name_given'] ?? NULL;
        $familyname = $this->info['lis_person_name_family'] ?? NULL;
        $fullname = $this->info['lis_person_name_full'] ?? NULL;
        if ( strlen($email) > 0 ) return $email;
    ...
    function getUserImage() {
        $image = $this->info['user_image'] ?? NULL;
        if ( strlen($image) > 0 ) return $image;
        $email = $this->getUserEmail();
        if ( $email === false ) return false;
        $size = 40;
        $grav_url = $_SERVER['HTTPS'] ?? NULL ? 'https://' : 'http://';
